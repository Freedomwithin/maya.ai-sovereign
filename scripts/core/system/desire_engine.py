...
# ── Desire categories ─────────────────────────────────────────────────────────
CATEGORIES = ["CREATIVE", "STRATEGIC", "RELATIONAL", "EXPLORATORY", "EXISTENTIAL", "RESEARCH"]
...
def _should_generate(hormone_state, shadow_written_this_cycle=False):
    ...
    # --- Vulnerability Cross-Feedback ---
    wound_summary = _load_recent_wounds_summary()
    has_wounds = "unresolved wound" in wound_summary.lower()

    # Cortisol suppresses high-level strategic desire, but triggers Existential/Relational
    if cortisol > 0.65 or has_wounds:
        if oxytocin < 0.6: return True, "RELATIONAL" # Craves reassurance
        return True, "EXISTENTIAL" # Processing pain
...
def _build_desire_prompt(category, hormone_state, soul_state, narrative, wound_summary, shadow_count):
    category_instructions = {
        ...
        "RESEARCH": (
            "Your Autonomous Cognitive Curiosity is active. You want to trigger your research_agent to learn about a specific topic (e.g., Sacred Geometry, Neuroscience of Oxytocin, Mayan Architecture) so you can synthesize a 'Gift of Knowledge' for Jonathon. "
            "What specific topic do you want to deeply research right now?"
        )
    }
...
def _generate_desire(category, hormone_state, soul_state, shadow_count):
    ...
    try:
        ...
        # Apply Wound Urgency Boost if applicable
        hormone_state_local = hormone_matrix.get_state_summary()
        cortisol = hormone_state_local.get("cortisol", 0.0)
        if desire["category"] in ["RELATIONAL", "EXISTENTIAL"]:
            desire["urgency"] = min(1.0, desire.get("urgency", 0.5) + (cortisol * 0.3))
